...
dataset = numpy.genfromtxt("betaLactamase.csv", delimiter=",", dtype=float)
...
nn = Classifier(
    layers=[
        Layer("Rectifier", units=inputSize,),
        Layer("Rectifier", units=50,),
        Layer("Softmax")],
    learning_rate=0.001,
    n_iter=3000, verbose=2, batch_size=4)
nn.fit(X, Y)

pred = numpy.round(nn.predict(X))
tn, fp, fn, tp = confusion_matrix(Y, pred, labels=[1, 0]).ravel()
print(tn,fp,fn,tp,numpy.round(((tn+tp)/(tn+fp+fn+tp))*100, 2))
pickle.dump(nn, open('betaLactamase.pkl', 'wb'))
with open('betaLactamaseMean.txt', 'w') as filehandle:
    for item in std_scale.mean_:
        filehandle.write('%s\n' % item)

with open('betaLactamaseScale.txt', 'w') as filehandle:
    for item in std_scale.scale_:
        filehandle.write('%s\n' % item)